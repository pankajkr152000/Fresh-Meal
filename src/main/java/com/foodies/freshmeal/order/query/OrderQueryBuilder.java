 ...
 *
 * <p>
 * This component translates the application-level
 * {@link OrderSearchRequest} into MongoDB {@link Query} objects without
 * exposing MongoDB field names to the API layer.
 * </p>
 *
 * <p>
 * Responsibilities:
 ...
 *
 * <p>
 * Pagination is intentionally not applied here. The service layer can use
 * the same criteria query for both count and data retrieval while applying
...
public class OrderQueryBuilder {
    ...
    public Query build(final OrderSearchRequest request) {
        ...
        applyDeliveryStatusFilter(
                query,
                request.getDeliveryStatuses());

        applyPaymentStatusFilter(
                query,
                request.getPaymentStatuses());
     ...
     *
     * <p>
     * Search is performed against the fields intentionally exposed as
     * searchable by the Order Management use case:
     * </p>
     *
     * <ul>
     * <li>Order number</li>
    ...
    private void applySearch(
        ...
        /*
         * Pattern.quote prevents user-provided search text from being treated
         * as a regular-expression expression.
         */
        final String regex = Pattern.quote(keyword);

        final Criteria searchCriteria = new Criteria().orOperator(

                Criteria.where(FIELD_ORDER_NUMBER)
                        .regex(regex, "i"),

                Criteria.where(FIELD_CUSTOMER_NAME)
                        .regex(regex, "i"),

                Criteria.where(FIELD_CUSTOMER_MOBILE)
                        .regex(regex, "i"));
     ...
     */
    private void applyDeliveryStatusFilter(
            final Query query,
            final List<String> statuses) {
     ...
     */
    private void applyOrderTypeFilter(
            final Query query,
    ...
    /**
     * Adds an IN criterion when the supplied values contain at least one
     * non-blank value.
     *
     * @param query  MongoDB query
    ...
    private void addInCriteria(
        ...
        final List<String> normalizedValues = values.stream()
    ...
    private void applyCustomerFilter(
        ...
        query.addCriteria(
                Criteria.where(FIELD_CUSTOMER_ID)
                        .is(customerId.trim()));
    }
    ...
    private void applyRestaurantFilter(
        ...
        query.addCriteria(
                Criteria.where(FIELD_RESTAURANT_ID)
                        .is(restaurantId.trim()));
    }
