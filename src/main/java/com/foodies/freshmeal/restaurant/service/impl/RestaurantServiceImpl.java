 ...
 * • Restaurant update
 * • Restaurant identifier generation
 * • Restaurant image reference validation
...
public class RestaurantServiceImpl implements IRestaurantService {
     ...
     *
     * <p>
     * Supplied logo and cover image identifiers are validated through the
    ...
    public IServiceOutput<RestaurantDetailsResponse> create(
            IServiceInput<RestaurantCreateRequest> input) {

        Objects.requireNonNull(
                input,
                "Restaurant service input must not be null.");

        RestaurantCreateRequest request = input.getInput();
        ...
        validateImageReference(request.getLogoImageId());

        validateImageReference(request.getCoverImageId());
        ...
        RestaurantDetailsResponse response = restaurantMapper.toDetailsResponse(
                restaurant);
    ...
    public IServiceOutput<List<RestaurantListResponse>> getAll(
        ...
        List<RestaurantListResponse> responses = restaurants.stream()
     ...
     *
     * <p>
     * The exact update operation will be implemented according to the
     * finalized RestaurantUpdateRequest contract.
     * </p>
     *
    ...
    public IServiceOutput<RestaurantDetailsResponse> update(
            IServiceInput<RestaurantUpdateRequest> input) {

        throw new UnsupportedOperationException(
                "Restaurant update implementation requires the finalized "
     ...
     *
     * <p>
     * Restaurant images are optional. When an image identifier is supplied,
     * the existing ImageService is used to verify that the image exists.
     * </p>
     *
    ...
    private void validateImageReference(
        ...
        IServiceInput<String> imageInput = new com.foodies.freshmeal.common.io.service.impl.ServiceInput<>();

        imageInput.setInput(imageId);

        IServiceOutput<ImageEntity> imageOutput = imageService.getImageEntityById(
                imageInput);
