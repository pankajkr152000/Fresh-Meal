...
public class AuditServiceImpl implements IAuditService {
	...
	private static final Set<String> SENSITIVE_FIELDS = Set.of("password", "passwd", "pwd", "otp", "token",
			"accessToken", "refreshToken", "jwt", "cvv", "cardNumber");

	private static final Logger LOGGER = LoggerFactory.getLogger(AuditServiceImpl.class);
	private final IAuditRepository repository;
	private final ObjectMapper objectMapper;

	public AuditServiceImpl(IAuditRepository repository, ObjectMapper objectMapper) {
	...
	@Override
	public void logSuccess(String api, MethodType method, Object request, Object response, Integer statusCode,
	...
	@Override
	public void logFailure(String api, MethodType method, Object request, Exception exception, Integer statusCode,
			Long executionTimeMs) {
	...
	public IAuditLog getAuditLogById(String id) {
		...
		return (IAuditLog) repository.findById(id).orElse(null);
	}

	@Override
	public List<IAuditLog> getAuditLogsByRequestId(String requestId) {

		return Collections
				.unmodifiableList(repository.findByRequestId(requestId).stream().map(log -> (IAuditLog) log).toList());
	}
	...
	private Document convertToDocument(Object object) {
		...
		try {
			...
			return new Document("error", "Unable to serialize object");
		}
	}
